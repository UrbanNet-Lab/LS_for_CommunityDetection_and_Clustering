...
def plot_combination1(x,y,text,x1,y1,text1,center_id,subplot_location,xlim_start_end,ylim_start_end,font_location,filepath='./',dataname='LS_default',save=False,write_center_name = True):
    '''
    input:
        x：节点的度值(数据类型：list)k
    ...
    ax.scatter(x, y, c=basecolor, marker='o',s=200,edgecolor=edgecolor)
    if write_center_name == True:
        for i in range(len(x)):
    #         ax.scatter(x[i], y[i], c=basecolor, marker='o',s=200,edgecolor=edgecolor)
            if text[i] in center_id:
...
#     left, bottom, width, height = 0.25,0.55,0.35,0.3 # Beijing
    ...
    ax1.scatter(x1_new, y1_new, c=basecolor, marker='o',s=2,edgecolor=edgecolor)
    if write_center_name == True:
        for i in range(len(x1_new)):
            if text1[i] in center_id:
                ax1.scatter(x1_new[i], y1_new[i], c=basecolor, marker='^',s=20,edgecolor=edgecolor)
#         else:
